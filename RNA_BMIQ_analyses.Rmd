...
for (i in names(FitList)) {
  ...
  enhanced_volcano[[i]] <- EnhancedVolcano(
    ...
    FCcutoff = 0.1,
    pCutoff = 0.0001,
    title = paste0("Volcano plot of the differential\nmethylation of CpGs ", i),
    subtitle = NA,
...
for (i in comparisons) {
  ...
  Genes_neighbor[[i]] <- match_hit_CpGs_neighbor_promoter[[i]] %>% dplyr::filter(., gene_names !="") %>% merge(., DGE_comparison_of_interest[[i]], by.x = "gene_names", by.y = "hgnc_symbol", all.x = TRUE) %>% dplyr::select(., gene_names, ID.x, logFC.x, logFC.y, P.Value)
  colnames(Genes_neighbor[[i]]) <- c("gene_names", "ID", "CpG_logFC", "Gene_logFC", "Gene_P.value")
...
for (i in comparisons) {
  ...
  Genes_potentially_down_regulated_by_methylation[[i]] <- Connexion_genes_cpgs[[i]] %>% dplyr::filter(., ID %in% V(net_Genes_down_cpgs_up)$name)
  Genes_potentially_up_regulated_by_unmethylation[[i]] <- Connexion_genes_cpgs[[i]] %>% dplyr::filter(., ID %in% V(net_Genes_up_cpgs_down)$name)
