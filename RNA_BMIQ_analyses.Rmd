...
png(filename = "Results/heatmap_BMIQ_no_M2_patients.png", width = 1920, height = 1080)
heat_no_M2_patients
dev.off()
```
...
dev.off()
png(filename = paste0("Results/heatmap_RNAseq_single_mutation.png"), width = 1920, height = 1080)
heat_single_mutation
dev.off()

no_wild_type <- design[,10] == 0
heat_no_WT <- Create_heatmap_corr(matrix_rna, no_wild_type, Phenotype_RNAseq)
png(filename = paste0("Results/heatmap_RNAseq_no_wild_type.png"), width = 1920, height = 1080)
heat_no_WT
dev.off()
no_wild_type_single_mutation <- rowSums(design[,c(1:4,7, 10)]) == 0
heat_no_WT_single_mutation <- Create_heatmap_corr(matrix_rna, no_wild_type_single_mutation, Phenotype_RNAseq)
png(filename = paste0("Results/heatmap_RNAseq_no_wild_type_single_mutation.png"), width = 1920, height = 1080)
heat_no_WT_single_mutation
dev.off()
...
png(filename = paste0("Results/heatmap_RNAseq_no_M2_patients.png"), width = 1920, height = 1080)
heat_no_M2_patients
dev.off()
```
...
```{r}
Connexion_genes_cpgs <- list()  
Genes_neighbor <- list()
for (i in comparisons) {
  Connexion_genes_cpgs[[i]] <- match_hit_CpGs_neighbor_promoter[[i]][match_hit_CpGs_neighbor_promoter[[i]]$gene_names %in% Genes_DE[[i]]$hgnc_symbol,] %>% merge(., Genes_DE[[i]], by.x = "gene_names", by.y = "hgnc_symbol", all.x = TRUE)
  colnames(Connexion_genes_cpgs[[i]]) <- c("gene_names", "ID", "CpG_logFC", "Gene_logFC", "AveExpr", "P.Value")
  message(paste0("\n[------------------------------------------- Comparison: ", i, " in progress -------------------------------------------]"))
  message("\n=== Genes DOWN expressed with CpGs Up methyl ===")
  Genes_DoE_UM <- Connexion_genes_cpgs[[i]] %>% dplyr::filter(., CpG_logFC > 0 & Gene_logFC < 0)
  ...
  Genes_UPE_DoM <- Connexion_genes_cpgs[[i]] %>% dplyr::filter(., CpG_logFC < 0 & Gene_logFC > 0)
  ...
  Genes_DE_neighbor <- Connexion_genes_cpgs[[i]]
  print(length(Genes_DE_neighbor$gene_names))
  write.csv(Genes_DE_neighbor, paste0("Results/", i, "/", i, "_genes_DE_neighbor_DM.csv"))
  
  Genes_neighbor[[i]] <- match_hit_CpGs_neighbor_promoter[[i]] %>% dplyr::filter(., gene_names !="") %>% merge(., DGE_comparison_of_interest[[i]], by.x = "gene_names", by.y = "hgnc_symbol", all.x = TRUE)
  message("\n=== Genes differentially expressed in the neighbor of CpGs ===")
...
for (i in comparisons) {
  ...
  subnet_df[[i]] <- as_long_data_frame(subnetwork[[i]])
}
